...
.ops-auth-shell::before{content:"";position:absolute;inset:0;z-index:-1;background:linear-gradient(90deg,rgba(9,13,36,.32),rgba(9,13,36,.62) 52%,rgba(9,13,36,.9)),linear-gradient(180deg,rgba(9,13,36,.18),rgba(9,13,36,.5))}
.ops-auth-card{width:min(100%,390px);padding:34px;text-align:center;border-radius:26px;background:linear-gradient(145deg,rgba(35,45,82,.86),rgba(17,23,53,.74));border:1px solid rgba(209,218,255,.18);box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 34px 100px rgba(0,0,0,.48);backdrop-filter:blur(30px) saturate(150%);-webkit-backdrop-filter:blur(30px) saturate(150%)}
@media(min-width:1000px){.qcards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));align-items:start}}
@media(max-width:520px){.wrap{padding:12px 10px 64px}.glass-card{border-radius:16px}.driver-access-grid{grid-template-columns:1fr}.ops-auth-shell{background-position:19% center}.ops-auth-shell::before{background:linear-gradient(180deg,rgba(9,13,36,.48),rgba(9,13,36,.84))}.ops-auth-card{padding:28px 22px;border-radius:22px}.ocard,.glass-card.ocard{padding:14px;border-radius:18px}.ocard-identity{grid-template-columns:40px minmax(0,1fr) auto;gap:10px}.ocard-kind{width:40px;height:40px}.ocard-name{font-size:1.05rem}.order-progress{padding-inline:0}.order-progress-label{font-size:.55rem}.ocard-actions .btn.sm{flex:1 1 calc(50% - 6px)}.inline-price{flex-direction:column}}
...
function loginHtml(){document.getElementById('app').innerHTML='<div class="ops-auth-shell"><div class="glass-card ops-auth-card"><div style="width:64px;height:64px;border-radius:18px;background:linear-gradient(135deg,#5b2a86,#8066e8);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:inset 0 1px 0 rgba(255,255,255,.18),0 16px 34px rgba(91,42,134,.32)"><span class="material-symbols-outlined" style="font-size:32px;color:#fff">two_wheeler</span></div><h2 style="color:#dfb7ff;font-size:1.3rem;margin-bottom:6px;font-family:Hanken Grotesk,sans-serif;font-weight:700">EdenMish Ops</h2><p class="muted" style="margin-bottom:20px;font-size:.85rem">מרכז הבקרה · תל אביב וגוש דן</p><input id="pin" type="password" placeholder="הזן PIN" style="text-align:center;font-size:1.2rem;letter-spacing:6px;margin-bottom:16px;width:100%"><button class="btn" style="width:100%" onclick="doLogin()">התחבר</button></div></div>';}
