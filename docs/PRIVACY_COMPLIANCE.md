...
- Meta/WhatsApp: optional zero-component delivery-complete templates only after
  a separate, persisted phone-channel opt-in and timestamp. Proof URLs, tracking
  tokens, order/customer details, and free text remain outside provider payloads;
  delivery proof stays in the private email/tracking channels.
