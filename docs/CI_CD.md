...
feat/*   → <branch>.edenmish-v2.pages.dev (auto-preview)
```

**Workflow:**
1. Create `feat/xxx` from `develop`, commit with conventional messages (`feat:`, `fix:`)
2. Open PR to `develop` → auto-preview URL for isolated testing
3. Merge to `develop` → `staging.edenmish.com` updated for integration testing
4. When ready: `git checkout main && git merge develop && npm version patch && git push`
5. Push to `main` → `edenmish.com` updated (production)

**Versioning:** Manual `npm version patch|minor` before merging to `main`.
Build stamp is auto-generated by `scripts/inject-version.js`.
...
- Storefront: `node --check` on extracted JS from booking.html, dash.html, etc.
